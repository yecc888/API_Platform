...
class BaseModel(models.Model):
    ...
    """
    m_time = models.DateTimeField(auto_now=True, verbose_name='更新时间', help_text='更新时间')
...
class Group(models.Model):
    ...
    分组
    """
    name = models.CharField(max_length=20, null=True, blank=True, verbose_name="分组名称")
    group = models.ForeignKey("self", null=True, on_delete=models.SET_NULL, blank=True, verbose_name="上级分组",
                              related_name='group')
...
class Headers(BaseModel):
    """接口模板"""
    name = models.CharField(max_length=124, default='', verbose_name='Header名称')
    value = models.CharField(max_length=124, default='', verbose_name='Header值')
    add_user = models.ForeignKey(User, null=True, on_delete=models.SET_NULL, blank=True, verbose_name='添加人')
...
class ApiManagerment(models.Model):
    ...
    env = models.ForeignKey(EnvManager, null=True, blank=True, verbose_name="所属环境", help_text="所属环境",
                            on_delete=models.SET_NULL)
    model = models.ForeignKey(ModelManager, null=True, blank=True, verbose_name='模块名称', on_delete=models.SET_NULL)
    ...
    protocol = models.CharField(max_length=10, default='HTTP', choices=PROTOCOL_TPYE, null=True, blank=True,
                                verbose_name='协议类型',
                                help_text='协议类型')
    method = models.CharField(max_length=10, default='GET', null=True, blank=True, choices=METHOD_TPYE,
                              verbose_name='请求方法', help_text='请求方法')
    ...
    #                              on_delete=models.SET_NULL)
    status = models.SmallIntegerField(default=1, choices=INTTERFACE_STATUS, verbose_name='接口状态', null=True, blank=True,
                                      help_text='接口状态')
    sign = models.SmallIntegerField(default=0, choices=SIGN, null=True, blank=True, verbose_name='接口是否加密',
                                    help_text='接口是否加密')
...
class AllHeaders(models.Model):
    ...
    接口请求header
    """
    api = models.ForeignKey(ApiManagerment, null=True, blank=True, verbose_name='所属接口',
                            related_name="headers", on_delete=models.CASCADE)
    ...
    raw = models.TextField(max_length=1024, default='', blank=True, verbose_name='raw格式hearer')
...
class ApiParams(models.Model):
    ...
    接口请求参数
    """
    api = models.ForeignKey(ApiManagerment, null=True, on_delete=models.SET_NULL, blank=True, verbose_name='所属接口',
...
class ApiResponse(models.Model):
    ...
    RESULT_STATUS = (('success', '成功'),
                     ('danger', '失败'))
    api = models.ForeignKey(ApiManagerment, null=True, blank=True, verbose_name='所属接口',
                            on_delete=models.SET_NULL, related_name="response")
...
class CustomParameters(BaseModel):
    ...
    自定义变量
    """
    name = models.CharField(max_length=20, null=True, blank=True, verbose_name='变量名称')
    key = models.CharField(max_length=20, null=True, blank=False, verbose_name='变量key')
    value = models.CharField(max_length=126, null=True, blank=True, verbose_name='变量值')
    ...
    date_type = models.CharField(max_length=30, blank=True, null=True, verbose_name='日期格式类型')
    str_type = models.SmallIntegerField(default=1, blank=True, null=True, verbose_name='字符串格式类型')
    str_length = models.SmallIntegerField(blank=True, null=True, verbose_name='字符串长度')
    uuid_type = models.CharField(max_length=300, blank=True, null=True, verbose_name='UUID格式类型')
...
class ApiMock(BaseModel):
    ...
    method = models.CharField(max_length=10, default='GET', null=True, blank=True, choices=METHOD_TPYE,
                              verbose_name='请求方法', help_text='请求方法')
    mock_headers = models.TextField(null=True, blank=True, verbose_name='接口请求header', default='',
                                    help_text='请输入字典格式的请求头')
    mock_param = models.TextField(null=True, blank=True, verbose_name='请求参数', help_text='请输入字典格式的请求参数', default='')
    ...
    body_type = models.CharField(
        ...
        help_text="请选择请求体类型")
    mock_bodys = models.TextField(null=True, blank=True, verbose_name='请求体', help_text='请求体', default='')

    response_content = models.TextField(null=True, blank=True, verbose_name='Mock接口返回数据', default='',
                                        help_text='请输入字典格式的响应结果')
    status = models.SmallIntegerField(default=1, choices=INTTERFACE_STATUS, verbose_name='接口状态', null=True, blank=True,
                                      help_text='接口状态')
    desc = models.CharField(max_length=200, default='', blank=True, null=True,
                            verbose_name='接口描述', help_text='接口描述')
    mock_times = models.SmallIntegerField(null=True, blank=True, verbose_name='接口请求次数', default=0)
...
class CaseManagerment(BaseModel):
    ...
    case_group = models.ForeignKey(CaseGroup, null=True, blank=True, related_name='cases', on_delete=models.SET_NULL,
                                   verbose_name='用例所属分组', help_text='用例所属分组')
    env = models.ForeignKey(EnvManager, null=True, on_delete=models.SET_NULL, blank=True, verbose_name='所属环境')
    model = models.ForeignKey(ModelManager, null=True, blank=True, on_delete=models.CASCADE,
                              verbose_name='用例所属模块', help_text='用例所属模块')
    name = models.CharField(max_length=255, null=True, blank=True, verbose_name='用例名称', default='',
                            help_text='用例名称')
    caseLevel = models.CharField(max_length=126, choices=CaseLevel, blank=True,
                                 null=True, verbose_name='用例等级', help_text='用例等级')
    caseStatus = models.CharField(max_length=126, choices=CaseStatus, blank=True,
                                  null=True, verbose_name='用例状态', help_text='用例状态')
    caseType = models.CharField(max_length=126, choices=CaseType, blank=True,
                                null=True, verbose_name='用例类型', help_text='用例类型'
                                )
    create_user = models.ForeignKey(User, null=True, blank=True, on_delete=models.SET_NULL,
...
class CaseApi(models.Model):
    ...
    method = models.CharField(max_length=10, default='GET', null=True, blank=True, choices=METHOD_TPYE,
                              verbose_name='请求方法', help_text='请求方法')
    header_is_check = models.BooleanField(verbose_name='是否检查header', default=False)
...
class CaseStepParams(models.Model):
    """
    接口请求参数
    """
...
#     """
#
#     name = models.ForeignKey(CaseManagerment, null=True, blank=True, verbose_name='所属用例',
#                              on_delete=models.SET_NULL)
#     status_code = models.IntegerField(null=True, blank=True, verbose_name='状态码')
#     response_status = models.CharField(max_length=10, null=True, blank=True, verbose_name='返回状态')
#     response_header = models.TextField(null=True, blank=True, verbose_name='服务器响应头')
